...
class FTP {
    ...
    private $conn, $login_result, $logData, $ftpUser, $ftpPass, $ftpHost, $retry, $ftpPasv, $ftpMode, $verbose, $logPath, $createMask,$port;
    ...
    {
        $this->retry = (isset($o['reattempts'])) ? $o['reattempts'] : 5;
     ...
     * @return  void
     */
    public function conn($hostname,$port, $username, $password)
    {   
    ...
    function initConn()
    {
        $this->conn = ftp_connect($this->ftpHost, $this->port);
    ...
    public function fileDelete($path,$file)
    {
        $list=ftp_nlist($this->conn,"$path");
        if(in_array($file, $list)){
        if(@ftp_delete($this->conn, $path.'/'.$file)){
            return true;
        }
        }
    ...
    public function singleFileDelete($path)
    {
